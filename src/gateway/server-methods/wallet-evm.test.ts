...
describe("wallet evm handlers", () => {
  ...
  it("signs a transaction after approval", async () => {
    ...
    const signPromise = evmHandlers["wallet.evm.signTransaction"]({
      params: {
        tx: {
          chainId: 1,
      ...
      respond,
      context: context as unknown as Parameters<
        (typeof evmHandlers)["wallet.evm.signTransaction"]
      >[0]["context"],
      client: null,
      req: { id: "req-1", type: "req", method: "wallet.evm.signTransaction" },
      isWebchatConnect: noop,
    });
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        id,
        decision: "approve",
        signedTransaction: expect.stringMatching(/^0x/i),
